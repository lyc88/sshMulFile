...
public class Page
{
	...
	//每页记录条数
	private String[] pageSizeAry = { "15", "30", "50"};
	...
	public String getPageMenu()
	{
		...
		if (this.getTargetPage() > 1)
		{
			str.append("<a href=\"javascript:void(0)\" onclick='document.all(\"" + targetPageField + "\",0).value=1;document.forms[0].submit();'>" + pageFirst
					+ "</a>&nbsp;&nbsp;");
			str.append("<a href=\"javascript:void(0)\" onclick='document.all(\"" + targetPageField + "\",0).value=" + prev
					+ ";try{doBeforeSubmit();}catch(e){}finally{document.forms[0].submit();}'>" + pagePrevious + "</a>&nbsp;");
		}
		else
		{
			pageFirst = fontColorStyle1 + pageFirst + fontColorStyle2;
		...
		if (this.getTargetPage() < this.getPageCount())
		{
			str.append("<a href=\"javascript:void(0)\" onclick='document.all(\"" + targetPageField + "\",0).value=" + next
					+ ";try{doBeforeSubmit();}catch(e){}finally{document.forms[0].submit();}'>" + pageNext + "</a>&nbsp;&nbsp;");
		}
		else
		{
			pageNext = fontColorStyle1 + pageNext + fontColorStyle2;
		...
		if (this.getPageCount() > 1 && this.getTargetPage() != this.getPageCount())
		{
			str.append("<a href=\"javascript:void(0)\"  onclick='document.all(\"" + targetPageField + "\",0).value=" + this.getPageCount()
					+ ";try{doBeforeSubmit();}catch(e){}finally{document.forms[0].submit();}'>" + pageLast + "</a>&nbsp;&nbsp;");
		}
		else
		{
			pageLast = fontColorStyle1 + pageLast + fontColorStyle2;
		...
		//页面每一页的记录条数
		str.append("<SELECT onchange='document.all(\"" + pageSizeField + "\",0).value=this.value;document.forms[0].submit();'>");
		for (String s : pageSizeAry)
		{
			if (this.getPageSize() == Integer.parseInt(s))
			{
		...
		str.append("&nbsp;&nbsp;&nbsp;&nbsp;" + pagePageNum + " ");
		str.append("<SELECT size=1 name=Pagelist onchange='document.all(\"" + targetPageField
				+ "\",0).value=this.value;try{doBeforeSubmit();}catch(e){}finally{document.forms[0].submit();}'>");
		int coun = this.getPageCount();
		if (coun > 0)
		{
			for (int i = 1; i < this.getPageCount() + 1; i++)
	...
	public String getBootstrapPage() {
		...
		if (this.getTargetPage() == 1) {
			...
			str.append("<li><a href='javascript:document.all(\"" + targetPageField + "\",0).value=" + prev
					+ ";try{doBeforeSubmit();}catch(e){}finally{document.forms[0].submit();}'>&laquo;</a></li>");
		}
		...
		if (this.getTargetPage() <= 6) {
			...
			for (int i = 1; i <= pageLimit; i++) {
				if (i != this.getTargetPage()) {
					str.append("<li><a href='javascript:document.all(\"" + targetPageField + "\",0).value=" + i
							+ ";try{doBeforeSubmit();}catch(e){}finally{document.forms[0].submit();}'>" + i
							+ "</a></li>");
				} else {
					str.append("<li class='active'><a href='javascript:document.all(\"" + targetPageField
							+ "\",0).value=" + i
							+ ";try{doBeforeSubmit();}catch(e){}finally{document.forms[0].submit();}'>" + i
							+ "</a></li>");
				}
		...
		if (this.getTargetPage() >= this.getPageCount()) {
			...
			str.append("<li><a href='javascript:document.all(\"" + targetPageField + "\",0).value=" + next
					+ ";try{doBeforeSubmit();}catch(e){}finally{document.forms[0].submit();}'>&raquo;</a></li>");
		}
